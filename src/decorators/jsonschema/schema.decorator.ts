 ...
 */
function modelFactory(model: any, options: any = {}) {
    ...
    for (let i in descr) {
        if (descr[i].$ref) {
            descr[i] = { $ref: descr[i].$ref };
        }//removes all other properties than $ref
...
export function Definitions(...defs: any[]) {
    return (model: any) => {
        for (let def of defs) {
